...
const NoticeListPage = () => {
  ...
  const reviewWriteAuthority =
    userId && (permission === "1" || permission === "2");

  //글 등록 페이지로 이동
  const goToNoticeWrite = () => {
    if (reviewWriteAuthority) {
      navigate("/news/noticeWrite");
    }
  };
  // const goToNoticeWrite = () => {
  //   navigate("/news/noticeWrite");
  ...
  return (
    <div>
      ...
      <div style={{ display: reviewWriteAuthority ? "block" : "none" }}>
        <Button type="middle" onClick={goToNoticeWrite}>
          공지작성
        </Button>
      </div>
      {/* <div>
